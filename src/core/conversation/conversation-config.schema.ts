...
export const ConversationSummarySchema = z.object({
    summary: z.string().describe("Concise 1-3 sentence overview of the entire conversation capturing the main topics discussed, issues addressed, and final outcomes"),
    key_points: z.array(z.string()).describe("Array of bullet points highlighting important discussion topics, decisions made, information exchanged, or concerns raised during the conversation"),
    action_items: z.array(z.string()).describe("Array of specific follow-up tasks, commitments, or next steps identified during the conversation (e.g., 'Send pricing quote by Friday', 'Schedule technical support callback', 'Update account information')"),
    sentiment: z.enum(ConversationSummarySentiment).describe("Overall emotional tone and customer satisfaction level detected from the conversation analysis (POSITIVE: satisfied/happy, NEGATIVE: frustrated/upset, NEUTRAL: informational, MIXED: varying emotions)")
});
...
export const MessageSchema = z.object({
    id: z.string().optional().describe("Optional internal database ID for the message record, auto-generated by the storage system"),
    messageId: z.string().describe("Unique identifier for this message used for deduplication, reference tracking, and message threading (typically UUID format)"),
    content: z.string().describe("Text content of the message sent by the user or assistant, may include plain text or formatted content depending on channel capabilities"),
    message_type: z.enum(MessageType).describe("Type of message sender indicating who sent this message: 'user' for customer messages, 'assistant' for AI agent responses"),
    timestamp: z.date().describe("JavaScript Date object representing the exact time when the message was created and sent in the conversation"),
    metadata: z.record(z.string(), z.any()).describe("Additional message-specific metadata as key-value pairs including attachments, formatting, media URLs, delivery status, read receipts, or channel-specific attributes"),
    created_at: z.number().optional().describe("Unix timestamp in milliseconds when the message was created, used for chronological sorting and time-based filtering in queries")
});
...
export const CallTransferSchema = z.object({
    transfer_type: z.enum(['blind', 'warm']).nullable().optional().describe("Type of call transfer: 'blind' for immediate unattended transfer without introduction (faster), 'warm' for attended transfer with agent briefing before connection (professional, allows screening)"),
    transfer_target: z.string().describe("Target phone number for the call transfer in E.164 format (e.g., '+12125551234') or internal extension identifier, must match CallTransferConfig.transfer_number"),
    transfer_initiated_at: z.number().optional().describe("Unix timestamp in milliseconds when the transfer was initiated by the AI agent, used for measuring time-to-transfer and transfer latency"),
    transfer_completed_at: z.number().optional().describe("Unix timestamp in milliseconds when the transfer was successfully completed with caller connected to human agent, used for calculating transfer duration"),
    transfer_status: z.enum(['pending', 'completed', 'failed', 'returned']).describe("Current status of the call transfer (pending: initiated awaiting answer, completed: successful connection, failed: no answer/busy/error, returned: call returned to AI after consultation)"),
    transfer_reason: z.string().optional().describe("Reason for the transfer as extracted from the conversation context (e.g., 'Billing inquiry requiring account access', 'Customer requested human agent', 'Technical issue beyond AI scope')")
});
...
export const ServiceConversationConfigSchema = BaseConversationConfigSchema.safeExtend({
    id: z.string().describe("Unique identifier for this conversation record (typically UUID), used as primary key for database queries and cross-system references"),
    record_id: z.string().optional().nullable().describe("Optional external record ID for integration with partner systems, CRM platforms, or help desk software for conversation linkage and data synchronization"),
    provider_message_id: z.string().optional().nullable().describe("Provider-specific email message ID from email service providers (Gmail message ID, Outlook message ID, SendGrid ID) for email conversation tracking and threading"),
    direction: z.enum(ConversationDirection).nullable().optional().describe("Direction of the telephony call: 'inbound' for customer-initiated calls, 'outbound' for agent-initiated or campaign calls (null for non-voice channels like chat/email)"),
    resource_url: z.string().nullable().optional().describe("Resource URL for the call recording or conversation details from telephony provider (SignalWire recording URL, Twilio media URL) for compliance, quality assurance, and dispute resolution"),
