...
export const TranslationServiceRequestSchema = BaseModelSchema.safeExtend({
    initiator_id: z.string().describe("Unique identifier for the initiator participant who is requesting the translation service (typically the customer or caller starting the session)"),
    initiator_language_code: LanguageCodeSchema.describe("ISO 639-1 language code of the initiator's native spoken language (e.g., 'en-US' for American English, 'es-MX' for Mexican Spanish, 'fr-FR' for French)"),
    participant_language_code: LanguageCodeSchema.describe("ISO 639-1 language code of the other participant's native spoken language (e.g., 'ja-JP' for Japanese, 'de-DE' for German, 'zh-CN' for Mandarin Chinese)"),
    session_id: z.string().optional().nullable().describe("Optional session ID for tracking and grouping related translation requests in multi-participant scenarios or session resumption"),
    translation_config_id: z.string().optional().nullable().describe("Optional translation configuration ID to use specific provisioning chain for this translation service (references ProvisioningConfigChain with isTranslation=true)"),
});
...
export const TranslationConversationConfigSchema = z.object({
    sdrtn_id: z.string().optional().nullable().describe("SDRTN (Session Description and Real-Time Networking) identifier for WebRTC connections enabling peer-to-peer audio streaming between participants"),
    channel_identifier: z.string().describe("Unique identifier for the communication channel (phone number in E.164 format, WebRTC channel ID, or conference room identifier) used for routing and session tracking"),
    initiator_access_id: z.string().describe("Unique access identifier for the initiator participant used to authenticate their WebRTC connection and track their audio stream"),
    initiator_token: z.string().describe("Short-lived authentication token for the initiator's communication channel, grants access to the translation session (expires after session or timeout)"),
    participant_access_id: z.string().describe("Unique access identifier for the other participant used to authenticate their WebRTC connection and track their audio stream"),
    participant_token: z.string().describe("Short-lived authentication token for the participant's communication channel, grants access to the translation session (expires after session or timeout)"),
});
