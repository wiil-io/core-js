...
export const BaseChatMessageSchema = z.object({
    conversation_config_id: z.string().describe("ID of the conversation configuration this message belongs to (N:1 relationship, references ServiceConversationConfig for conversation context and metadata)"),
    message: z.string().describe("Text content of the message sent by user or generated by agent, may include plain text or markdown formatting depending on channel capabilities"),
    timestamp: z.number().default(Date.now()).describe("Unix timestamp in milliseconds when the message was created and sent (default: current time, used for chronological ordering and latency analysis)"),
    llm_conversation_id: z.string().optional().nullable().describe("LLM provider conversation thread ID for multi-turn context tracking (OpenAI thread ID, Anthropic conversation ID) enabling context retention across messages"),
});
...
export const BaseEmailMessageSchema = BaseChatMessageSchema.safeExtend({
    subject: z.string().describe("Subject line of the email message used in email clients and for email thread grouping, typically maintained across conversation for proper threading"),
    isEmail: z.literal(true).default(true).describe("Flag indicating this is an email message (always true), distinguishes email messages from chat messages in unified message collections"),
});
...
export const UserChatMessageSchema = BaseChatMessageSchema.safeExtend({
    message_type: z.literal(MessageType.USER).default(MessageType.USER).describe("Message type identifier fixed to 'user' indicating this message was sent by the customer/end-user"),
    user_message_id: z.string().optional().nullable().describe("Unique identifier for this user message (typically UUID) used for message threading, deduplication, and reference in assistant responses"),
    last_assistant_message_id: z.string().optional().describe("ID of the last assistant message this user message is responding to, creates bidirectional message threading for conversation flow tracking"),
});
...
export const AssistantChatMessageSchema = BaseChatMessageSchema.safeExtend({
    message_type: z.literal(MessageType.AGENT).default(MessageType.AGENT).describe("Message type identifier fixed to 'assistant' indicating this message was generated by the AI agent"),
    assistant_message_id: z.string().describe("Required unique identifier for this assistant message (typically UUID) used for message threading, user response linking, and performance tracking"),
    last_user_message_id: z.string().optional().describe("ID of the user message this assistant message is responding to, creates bidirectional message threading and enables response latency measurement"),
});
...
export const UserEmailMessageSchema = BaseEmailMessageSchema.safeExtend({
    message_type: z.literal(MessageType.USER).default(MessageType.USER).describe("Message type identifier fixed to 'user' indicating this email was sent by the customer/end-user"),
    provider_message_id: z.string().optional().nullable().describe("Provider-specific email message ID from external email services (Gmail message ID, Outlook message ID, SendGrid message ID) for email tracking and reply threading"),
});
...
export const AssistantEmailMessageSchema = BaseEmailMessageSchema.safeExtend({
    message_type: z.literal(MessageType.AGENT).default(MessageType.AGENT).describe("Message type identifier fixed to 'assistant' indicating this email was generated by the AI agent"),
});
 ...
 */
export const HumanAgentChatMessageSchema = BaseChatMessageSchema.safeExtend({
    message_type: z.literal(MessageType.HUMAN_AGENT).default(MessageType.HUMAN_AGENT).describe("Message type identifier fixed to 'human_agent' indicating this message was sent by a human support agent"),
    agent_message_id: z.string().describe("Unique identifier for this human agent message (typically UUID) used for message threading and response tracking"),
    agent_session_id: z.string().describe("Agent workforce session ID linking this message to the human agent's active support session for workload tracking and analytics"),
    last_user_message_id: z.string().optional().describe("ID of the user message this human agent message is responding to, creates bidirectional message threading"),
});
 ...
 */
export const HumanAgentEmailMessageSchema = BaseEmailMessageSchema.safeExtend({
    message_type: z.literal(MessageType.HUMAN_AGENT).default(MessageType.HUMAN_AGENT).describe("Message type identifier fixed to 'human_agent' indicating this email was sent by a human support agent"),
    agent_session_id: z.string().describe("Agent workforce session ID linking this email to the human agent's active support session"),
});
 ...
 */
export const SystemMessageSchema = BaseChatMessageSchema.safeExtend({
    message_type: z.literal(MessageType.SYSTEM).default(MessageType.SYSTEM).describe("Message type identifier fixed to 'system' indicating this is a system-generated event message"),
    system_message_id: z.string().describe("Unique identifier for this system message (typically UUID) used for event tracking and audit trail reference"),
    event_type: z.enum(SystemMessageEventType).describe("Type of system event that triggered this message (handover, agent status, transfer events)"),
    metadata: z.record(z.string(), z.unknown()).optional().describe("Additional event-specific metadata as key-value pairs (agent ID, transfer reason, queue name, etc.)"),
});
