...
import { OrderAddress, OrderPricingSchema } from "../order.schema";
import { ExternalRefSchema, OrderStatus, PaymentStatus } from "../../type-definitions";
...
export const ProductOrderSchema = BaseModelSchema.safeExtend({
    locationId: z.string().nullable().optional().describe("Business location ID for multi-location businesses. Links order to specific warehouse/branch."),
    channelId: z.string().nullable().optional().describe("Channel ID for multi-channel orders (e.g., shopify, amazon, direct). Used for attribution and analytics."),
    ...
    orderNumber: z.string().nullable().default(null).describe("Human-readable order number displayed to customers and staff (e.g., 'ORD-12345', 'PO-2024-001'). Generated by Product Management for easy reference and communication."),
