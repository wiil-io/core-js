...
import { RoomRatePerNightSchema } from "./reservation-room.schema";
 ...
 */
export const ReservationLocalDateSchema = z
    .string()
 ...
 */
export const ReservationTimeOfDayValueSchema = z
    .string()
 ...
 */
export const ReservationSlotResourceTypeSchema = z.union([
    z.literal(ResourceType.TABLE),
    z.literal(ResourceType.ROOM),
    z.literal(ResourceType.RENTAL),
]);

const ReservationSlotQueryRequestBaseSchema = z.object({
    locationId: z.string().nullable().optional().describe("Optional business location ID used to scope slot generation."),
    resourceId: z.string().nullable().optional().describe("Optional resource or category ID used to scope slot generation."),
    localDate: ReservationLocalDateSchema.describe("Business-local date to generate slots for."),
    maxResults: z.number().int().positive().max(1000).default(20).describe("Maximum number of slots returned by the query."),
});
 ...
 */
export const TableReservationSlotQueryRequestSchema = ReservationSlotQueryRequestBaseSchema.extend({
    resourceType: z.literal(ResourceType.TABLE),
    partySize: z.number().int().positive().describe("Number of guests in the table reservation party."),
    floorPlanId: z.string().nullable().optional().describe("Optional floor plan ID used to scope table slot generation."),
    floorPlanSectionId: z.string().nullable().optional().describe("Optional floor plan section ID used as a table preference."),
});
 ...
 */
export const RoomReservationSlotQueryRequestSchema = ReservationSlotQueryRequestBaseSchema.extend({
    resourceType: z.literal(ResourceType.ROOM),
    nights: z.number().int().positive().default(1).describe("Requested stay length in nights."),
    occupancy: z.number().int().positive().nullable().optional().describe("Requested room occupancy."),
});
 ...
 */
export const RentalReservationSlotQueryRequestSchema = ReservationSlotQueryRequestBaseSchema.extend({
    resourceType: z.literal(ResourceType.RENTAL),
    returnDate: ReservationLocalDateSchema.nullable().optional().describe("Return date in YYYY-MM-DD format. Omit for intra-day rentals that use durationMinutes."),
    tierId: z.string().nullable().optional().describe("Requested rental tier ID from ReservationSettings."),
    durationMinutes: z.number().int().positive().nullable().optional().describe("Requested intra-day rental duration in minutes."),
});
 ...
 */
export const ReservationSlotQueryRequestSchema = z.discriminatedUnion("resourceType", [
 ...
 */
export const ReservationCandidateSlotSchema = z.object({
    resourceId: z.string().describe("Resource category or type ID represented by this candidate slot."),
    startTimeOfDay: ReservationTimeOfDayValueSchema.describe("Local start time in h:mm AM/PM format."),
    startMinuteOfDay: z.number().int().min(0).max(1439).describe("Local start minute within the business day."),
    endMinuteOfDay: z.number().int().min(1).max(1440).describe("Local end minute within the business day."),
    startTimeUtcSec: z.number().describe("Slot start timestamp in UTC seconds."),
    endTimeUtcSec: z.number().describe("Slot end timestamp in UTC seconds."),
    isAvailable: z.boolean().default(true).describe("Whether this slot can currently be booked."),
    capacityRemaining: z.number().int().nonnegative().nullable().optional().describe("Optional remaining capacity for capacity-based reservation modes."),
    priceQuote: z.number().nonnegative().nullable().optional().describe("Optional quoted price for the slot before final checkout."),
 ...
 * @property {boolean} isAvailable - Whether the slot can be booked
 */
export const RoomCandidateSlotSchema = z.object({
    resourceId: z.string().describe("Room type or category ID represented by this candidate slot."),
    checkInDate: ReservationLocalDateSchema.describe("Check-in date in YYYY-MM-DD format."),
    checkOutDate: ReservationLocalDateSchema.describe("Check-out date in YYYY-MM-DD format."),
    nights: z.number().int().positive().describe("Number of nights in the candidate stay."),
    occupancy: z.number().int().positive().nullable().optional().describe("Maximum guests supported by this room type."),
    roomsAvailable: z.number().int().nonnegative().describe("Number of rooms still bookable for the requested dates."),
    ratePerNight: z.array(RoomRatePerNightSchema).default([]).describe("Per-date nightly rates used to quote this stay."),
    totalWithTax: z.number().nonnegative().describe("Pre-computed total including tax."),
    isAvailable: z.boolean().default(true).describe("Whether this room slot can currently be booked."),
}).superRefine((slot, ctx) => {
 ...
 * @property {boolean} isAvailable - Whether the slot can be booked
 */
export const RentalCandidateSlotSchema = z.object({
    resourceId: z.string().describe("Rental resource type or category ID represented by this candidate slot."),
    pickupDate: ReservationLocalDateSchema.describe("Pickup date in YYYY-MM-DD format."),
    returnDate: ReservationLocalDateSchema.describe("Return date in YYYY-MM-DD format. Same as pickupDate for intra-day rentals."),
    pickupTimeOfDay: ReservationTimeOfDayValueSchema.nullable().optional().describe("Pickup time of day for intra-day rentals."),
    returnTimeOfDay: ReservationTimeOfDayValueSchema.nullable().optional().describe("Return time of day for intra-day rentals."),
    durationMinutes: z.number().int().positive().nullable().optional().describe("Duration in minutes for intra-day rentals."),
    startTimeUtcSec: z.number().describe("Pickup timestamp in UTC seconds."),
    endTimeUtcSec: z.number().describe("Return timestamp in UTC seconds."),
    unitsAvailable: z.number().int().nonnegative().describe("Number of rental units still available for the requested period."),
    totalWithTax: z.number().nonnegative().describe("Pre-computed total including tax."),
    priceQuote: z.number().nonnegative().nullable().optional().describe("Quoted price before tax."),
    isAvailable: z.boolean().default(true).describe("Whether this rental slot can currently be booked."),
}).superRefine((slot, ctx) => {
 ...
 */
export const TableReservationSlotQueryResponseSchema = z.object({
    resourceType: z.literal(ResourceType.TABLE),
    localDate: ReservationLocalDateSchema,
    generatedAt: z.number().describe("Generation timestamp in UTC seconds."),
    slots: z.array(ReservationCandidateSlotSchema).describe("Available candidate table slots."),
});
 ...
 */
export const RoomReservationSlotQueryResponseSchema = z.object({
    resourceType: z.literal(ResourceType.ROOM),
    checkInDate: ReservationLocalDateSchema.describe("Requested check-in date."),
    checkOutDate: ReservationLocalDateSchema.describe("Requested check-out date."),
    nights: z.number().int().positive().describe("Requested number of nights."),
    generatedAt: z.number().describe("Generation timestamp in UTC seconds."),
    slots: z.array(RoomCandidateSlotSchema).describe("Available candidate room slots."),
});
 ...
 */
export const RentalReservationSlotQueryResponseSchema = z.object({
    resourceType: z.literal(ResourceType.RENTAL),
    pickupDate: ReservationLocalDateSchema.describe("Requested pickup date."),
    returnDate: ReservationLocalDateSchema.describe("Requested return date."),
    generatedAt: z.number().describe("Generation timestamp in UTC seconds."),
    slots: z.array(RentalCandidateSlotSchema).describe("Available candidate rental slots."),
});
 ...
 */
export const ReservationSlotQueryResponseSchema = z.discriminatedUnion("resourceType", [
    TableReservationSlotQueryResponseSchema,
    RoomReservationSlotQueryResponseSchema,
    RentalReservationSlotQueryResponseSchema,
]);
