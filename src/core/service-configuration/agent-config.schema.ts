...
export const AgentConfigurationSchema = BaseModelSchema.safeExtend({
    ...
    name: z.string().max(30).describe("Personal name for the AI agent used in conversations (e.g., 'Sarah', 'James', 'Alex')"),
    ...
    useCustomModel: z.boolean().optional().default(false).describe("Whether to opt out of platform default model configurations and only use the specified modelId and requiredModelConfig"),
    textProcessingModelId: z.string().nullable().optional().describe("Specific model ID to use for text processing, if different from the main modelId"),
